...
function rsCompanyAssigned() {
  ...
  ${summaryStrip([["Phải thu kỳ này", formatMoney(due), `${companyAreas.length} tổ · ${companyAreas.reduce((t, a) => t + a.households, 0).toLocaleString("vi-VN")} hộ`], ["Đã thu", formatMoney(collected), `${due ? Math.round(collected / due * 100) : 0}% · công ty báo cáo`], ["Xã đã lập phiếu thu", formatMoney(remitted), `${receipts.length} phiếu thu`], ["Còn phải nộp về xã", formatMoney(Math.max(collected - remitted, 0)), collected - remitted > 0 ? "Đã thu chưa nộp" : "Đã nộp đủ"]])}
